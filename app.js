...
const items = Array.from({length:100},(_,i)=>{
  ...
  return {id:`RIFT-${String(n).padStart(3,'0')}`,index:String(n).padStart(2,'0'),title:featured[n%featured.length],skins,heroes,rune,rank,price,sold,rare:n%2===0,clean:n%3!==0,warranty:n%4!==0,win:48+((n*3)%19)};
});
...
function render(){
  ...
  let data=items.filter(x=>{
    const hay=`${x.id} ${x.title} ${x.rank}`.toLowerCase();
  ...
  $('#products').innerHTML=data.slice(0,shown).map(x=>`
    <article class="product">
      <div class="cover" data-index="${x.index}"><div class="cover-top"><span class="cover-code">${x.id}</span><span class="cover-state ${x.sold?'sold':''}">${x.sold?'SOLD':'AVAILABLE'}</span></div><div class="cover-bottom"><b>${x.title}</b><span>${x.rank} / Rune ${x.rune}</span></div></div>
...
function openDetail(id){
  ...
  openDrawer(`รายละเอียด ${x.id}`,`
    <div class="detail-cover"><span>${x.id} / ${x.rank}</span><strong>${x.title}</strong><span>${x.sold?'SOLD':'AVAILABLE'} · Mock product dossier</span></div>
    ...
    <div class="notice">ข้อมูลทั้งหมดเป็นข้อมูลจำลองสำหรับ Review UX/UI เท่านั้น รุ่น Production ต้องเชื่อม inventory จริงและมีการตรวจสอบสถานะบัญชีก่อนแสดงผล</div>`,
...
function openAuth(mode='login'){
  ...
  openDrawer(mode==='login'?'เข้าสู่ระบบ':'สมัครสมาชิก',`
    ...
    <div class="notice">Mock authentication — ปุ่มด้านล่างจะจำลองสถานะสมาชิกใน browser เท่านั้น</div>`,
...
function openCheckout(x){
  ...
  openDrawer('ตรวจสอบคำสั่งซื้อ',`
    ...
    ${payment==='slip'?'<div class="form-row"><label>แนบสลิป (Mock)</label><input type="file"></div>':'<div class="notice">Production version: ตรงนี้จะแสดง QR พร้อม order reference และสถานะการชำระเงินแบบ real-time</div>'}`,
